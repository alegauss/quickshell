...
- 📋 **QS19** (deps: QS18) **A remote host can write the local clipboard and read back a string it planted in the title** — Escape sequences are an input channel from a machine the user may not control, and the two that reach back out of the terminal both have a history of exploitation. → §QS19
...
- 📋 **QS85** (deps: —) **The commit script stages the whole tree, so a second session's half-written work lands inside another task's commit** — It already happened once in this checkout, and the history now credits one task with files that belonged to another. → §QS85
...
- **A reference image is never regenerated to make a test pass** Settled by reviewing
