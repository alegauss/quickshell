...
// template fails the build rather than shipping a page under another route's title. It reads
// the SSR bundle built by `vite build --ssr`, and the route metadata comes from the same
...
// The manifest lists the routes, their twins and their sizes, so an agent can find the flat
// files without running the page. No build timestamp: the same input builds byte-identical
// output.
const manifest = {
  name: "Quickshell",
  base: "/quickshell/",
 ...
 * every page did. Deliberately one date for every route rather than one per route, because
 * `lastmod` per URL would need to know which sources compose which page, and that is either
 * a module graph or a hand-kept table. One date over the whole authored tree is a weaker
...
// carries the base prefix, so a rename that moves every route moves this too.
